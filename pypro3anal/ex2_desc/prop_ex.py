# 비율 검정 : 집단의 비율이 어떤 특정한 값과 같은지를 검정

# one-sample
# A회사에서는 100명 중에 45이 흡연을 한다. 국가 통계에서는 35%로 알려져 있다고 할 때, 두 그룹 간 비율이 동일한가?
...
print(z)
print(p)        # 0.044 < 0.05이므로 귀무가설 기각. A회사의 흡연율과 국민 흡연율의 비율은 다르다.
...
print(z)
print(p)        # 0.0136 < 0.05 이므로 귀무가설 기각. 두 회사의 햄버거 취식 비율은 다르다.
