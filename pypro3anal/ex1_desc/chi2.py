...
'''
# 가설 검정은 수집된 데이터를 근거로 기존에 인식된 생각(귀무)을 기준으로 새롭게 제기된 주장을 대립 가설로 만들어 검증하는 것이다.
# 귀무 : 기대치와 관찰치는 차이가 없다. 주사위는 게임에 적합하다. 
# 대립 : 기대치와 관찰치는 차이가 있다. 주사위는 게임에 적합하지 않다.

# 실험에 의해 수집된 집단이 한 개이므로 일원 카이제곱 검정 실시
import pandas as pd 
import scipy.stats as stats 

obs_data = [4,6,17,16,8,9]
result = stats.chisquare(obs_data) # (f_obs, f_exp, ddof, axis) 관찰값, 기댓값, 자유도, 진행방향
print(result)   # statistic=14.200000000000001, pvalue=0.014387678176921308)
print('검정통계량: %.5f, p-value: %.5f'%(result))
# 해석1(카이제곱분포표)-> df:(6 - 1), c.v : 11.07 (카이제곱표에 의해), chi²: 14.2 이므로 귀무 기각
# 해석2(p값) : p-value: 0.01439 < 0.05 이므로 귀무 기각. 주사위는 게임에 적합하지 않다.
# 결국 위 검정은 관찰된 빈도가 기대되는 빈도와 유의한 차이가 있는지를 검증한다.
...
print(stats.chisquare(data1['관측도수']))
