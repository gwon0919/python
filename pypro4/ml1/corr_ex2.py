# 공분산은 두 개 이상의 확률변수에 대한 관계를 알려 주는 값이나, 값의 범위가 정해져 있지 않아 기준 값을 설정할 수 없다.
...
# 공분산 
# print(np.cov(data.친밀도, data.적절성, data.만족도))      # 에러 : 확률변수는 두개 
...
print(data.corr(method='spearman'))       # 서열 척도. 비정규분
print(data.corr(method='kendall'))           # spearman 과 유사
