...
# ROC curve, AUC도 사용
# 연구 보고서 주제를 설정 => 데이터 수집 및 가공 => 모델 생성 및 학습 => 모델을 평가
#  => 유의하다면 인사이트를 얻어 의사결정 자료로 활용
...
# 표본 100개, 독립변수 2개, n_redundant는 독립변수 중 다른 독립변수의 선형조합으로 나타낸 성분 수
print(x[:3], x.shape)
print(y[:3], y.shape)

# import matplotlib.pyplot as plt  # 안중요함
# plt.scatter(x[:,0], x[:,1])
# plt.show()
...
print('f_value : ', f_value[:10])  # 양수 값은 양성 클래스를 의미하며 음수 값은 음성 클래스를 의미한다.
...
print(confusion_matrix(y, y_hat))  # 실제값, 예측값
...
print('acc(정확도) : ', acc)
print('recall(재현률, 민감도, TPR) : ', recall)
print('precision(정밀도, 민감도, TPR) : ', precision)  # 전체 양성 자료 중에 양성으로 예측된 비율. 1에 가까울수록 좋음
print('specificity(특이도) : ', specificity)
print('fallout(위양성률, FPR) : ', fallout)  # 전체 음성 자료 중에 양성으로 잘못 예측된 비율. 0에 가까울수록 좋음
