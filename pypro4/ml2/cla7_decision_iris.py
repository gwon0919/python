...
x = iris.data[:,[2, 3]]  # feature : 'petal length (cm)', 'petal width (cm)'  matrix
y = iris.target  # vector
print(x[:2])
print(y[:2], set(y))  # species : {0, 1, 2} 3가지로 나뉘어짐
...
print()
'''
# scaling(크기를 고르게) - feature에 대해 데이터 표준화, 정규화 : 최적화 과정에서 안정성, 수련 속도를 향상 시킴, 오버피팅 or 언더피팅 방지 기능
print(x_train[:3])
sc = StandardScaler()
sc.fit(x_train)
sc.fit(x_test)
...
print(x_train[:3])
# scaling을 train에 사용한 경우 모델을 이용해 값을 예측할 때도 scaled 데이터를 가지고 실행해야함

# scaling 값 원복
inver_x_train = sc.inverse_transform(x_train)
print(inver_x_train[:3])  # 정확한 원복이 아닐수도 있음
'''

# 모델 작성 --------------------------------------------------------------------
model = DecisionTreeClassifier(criterion='entropy', max_depth=3, random_state=0)
# C=() : 분류 정확도 조정 - L2 규제 (정규화) : 값이 작을 수록 규제가 강함
...
joblib.dump(model, 'cla4model.save')

del model  # 실습용

mymodel = joblib.load('cla4model.save')
print("새로운 값으로 분류 예측 - 'petal length (cm)', 'petal width (cm)' - scaling으로 학습했다면 예측 데이터도 scaling 하기")  # scaling : axis.transform
print(x_test[:2])
new_data = np.array([[5.1, 2.4], [0.1, 0.1], [5.6, 5.6], [8.1, 0.5]])
new_pred = mymodel.predict(new_data)  # softmax 함수가 제공한 결과에 대해 가장 큰 인덱스를 반환
print('예측 결과 : ', new_pred)  # [2 0 2 2] : index
print('softmax 결과(날것) : ', mymodel.predict_proba(new_data))

# 이항분류는 sigmoid, 다항분류는 softmax *****

# 시각화
def plot_decision_region(X, y, classifier=mymodel, test_idx=None, resolution=0.02, title=''):
...
graph.write_png('cla7iris.png')

from matplotlib.pyplot import imread
img = imread('cla7iris.png')
plt.imshow(img)
plt.show()
