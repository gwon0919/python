# DecisionTree(의사결정 나무) : classification, regression 둘 다 가능. 주로 분류에서 사용.
# 데이터 균일도에 따른 규칙기반의 결정 트리.
# 간단한 알고리즘에 비해 성능이 우수하다. 데이터의 양이 많아질수록 성은은 떨어짐.  
...
# entropy : 데이터 집합에서 혼잡도(불순도)를 말하며 클수록 불순도가 높다. 불확실성을 나타내는 척도
print(model)
