# with 구문
# with 명령문 as 변수명 : ~
# 어떤 블록에 진입하고 나올때 지정된 객체(context manager)로 하여금 그 시작과 끝에서 어떤 처리를 할 수 있다.
# file i/o에서 with문을 사용하면 close()를 자동으로 설정
...
try:
    ...
    tupleData = (dictData,listData)               # 복합객체
    print(tupleData)
    
    print('-----저장------')
    with open('hello.obj', mode='wb') as f1:
        pickle.dump(tupleData, f1)              # 복합객체 저장
        pickle.dump(listData, f1)               # 일반객체 저장
    print('피클을 이용한 객체 저장')
