...
# 일급함수를 : 함수안에 함수 선언 가능, 인자로 함수를 전달, 반환값이 함수 인 경우 
from anaconda_project.internal.conda_api import result
from scipy.stats.tests.test_generation.studentized_range_mpmath_ref import result_exists
...
# 문1) 1 ~ 100 사이의 정수 중 5의 배수이거나 7의 배수만 걸러 보기
print(list(filter(lambda a:a %5==0 or a%7==0 , range(1,101))))

print('함수 장식자 : @함수명-meta 기능을 갖음')
# 장식자는 다르 함수를 감싼 함수다. 주 함수가 호출되면 그 반환 값이 장식자에게 건네진다.
...
print('계승(factorial : 1부터 어떤 양의 정수 n까지의 수를 모두 곱한것) 처리를 위한 함수')
def myFact(a):
    if a == 1:return 1
    print(a)
    return a * myFact(a - 1)
